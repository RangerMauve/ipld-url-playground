...
export default class IPLDURL extends URL {
  ...
  get segments () {
    return this.pathname.split('/').map(pathSegment => {
      ...
      const rawName = pathSegment.replace(IPLD_LENSE_SPECIFIER_REGEX, '')
  ...
  set segments (segments) {
    ...
    this.pathname = segments.map(segment => {
      if (typeof segment === 'string') return encodeURIComponent(segment)
      ...
      if (segment.name) {
        segmentContent += encodeURIComponent(segment.name)
      }
